...
int GetUpgradeCost(byte Nat,GAMEOBJ* Upgrade,byte ResID){
	...
	tempaddrand(Upgrade->Index);
	tempaddrand(Nat);
	tempaddrand(NATIONS[Nat].UPGRADE[Upgrade->Index]->Cost[ResID]);
...
int GetMoney(byte id){
	...
	if(AiIsRunNow){
		if(id<6){
			tempaddrand(XRESRC(CurAINation,id));
			tempaddrand(CurAINation);
...
int GetUnits(GAMEOBJ* UnitType){
	if(AiIsRunNow){
		...
		tempaddrand(CCIT->UnitAmount[UnitType->Index]);
...
int GetReadyUnits(GAMEOBJ* UnitType){
	if(AiIsRunNow){
		...
		tempaddrand(CCIT->ReadyAmount[UnitType->Index]);
...
bool UpgIsDone(GAMEOBJ* Upgrade){
	if(AiIsRunNow){
		...
		tempaddrand(CNAT->UPGRADE[Upgrade->Index]->Done);
...
bool UpgIsRun(GAMEOBJ* Upgrade){
	if(AiIsRunNow){
		...
		tempaddrand(CNAT->UPGRADE[id]->Done||CNAT->UPGRADE[id]->IsDoing);
...
#ifdef MAKE_PTC
bool PLOAD=0;
SFLB_DLLEXPORT SFLB_LoadPDATA(char* Name){
	PLOAD=0;
	char cc[200];
	sprintf(cc,"Players\\%s.txt",Name);
	GFILE* F=Gopen(cc,"r");
...
#endif
#ifdef MAKE_PTC
SFLB_DLLEXPORT SFLB_LoadPlayerData(){
	DLL1(SFLB_LoadPDATA,PlName);
	if(!PLOAD)DLL1(SFLB_LoadPDATA,"standart player info");
};
#endif
WarPack WARS;
void InvBatt(){
	...
	if(f){
		...
		for(int i=0;i<NWars;i++){
			...
			for(int j=0;j<WAR->NBatles;j++){
				z=Gscanf(f,"%s",ccc);
				if(z!=1)InvWar();
				for(int k=0;k<NBattles&&strcmp(ccc,Battles[k].ID);k++);
				if(k<NBattles){
					WAR->BattleList[j]=k;
				}else{
					char cc1[150];
