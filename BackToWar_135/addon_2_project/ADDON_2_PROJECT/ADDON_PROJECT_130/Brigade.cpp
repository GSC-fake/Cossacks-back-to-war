...
void DrawBorder(Brigade* BR){
	if(BR->WarType&&BR->NMemb>2&&BR->PosCreated){
		...
		byte cl=clrYello;
		if(BR->BOrder){
			if(BR->BOrder->BLink==&B_KeepPositionsLink)Cross=true;
...
void ResearchCurrentIsland(byte Nat){
	...
	//delete it:
	addrand(NAreas);
	for(int f=0;f<NAreas;f++)addrand(TopIslands[f]);
	//-----------
	int NInIsl[MaxIsl];
		...
		if(BestID==0xFFFF){
			...
			for(i=0;i<EIN->NHSHIPS;i++){
				...
				if(DR<MAXR){
					...
					int y0=HS->y;
					for(int j=0;j<NS;j++){
						DR=Norma(XI[j]-x0,YI[j]-y0);
						if(DR<RMAX[j]&&DR>RMIN[j]){
							SFORCE+=DANG[j];
						};
					};
					if(SFORCE){
						int F=(HS->Life*1000)/SFORCE;
						if(F>MAXF){
							OneObject* OB=Group[HS->ID];
							if(OB&&!OB->Sdoxlo){
								MAXF=F;
