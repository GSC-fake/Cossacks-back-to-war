	...
	if(Port){
		...
		if(OR1&&OR1->DoLink==&ProduceObjLink){
			...
			if(NA->Enabled){
				...
				AddPoint(CX,CY,CX+NF->dx,CY+NF->dy,NULL,NF->FileID,NF->SpriteID,0,0);
				if(NA->NFrames>1){
					//reflection
					NF=&NA->Frames[1];
...
//	byte NI=OBJ->NNUM;
	...
	if(ID!=-1){
		...
		NewMonster* NM1=OB->newMons;
		{
			...
			//OB->PrioryLevel=16;
			for(int j=1;j<NM->NBornPt;j++){
				if(j==NM->NBornPt-1){
					//OB->ClearOrderedUnlimitedMotion(2);
					OB->NewMonsterSendTo(((xx+NM->BornPtX[j])<<8)/*+(rando()&1023)-512*/,((yy+NM->BornPtY[j])<<8)/*+(rando()&1023)-512*/,16,2);
				}
			...
			OB->NewMonsterSendTo(((xx+NM->BornPtX[j-1])<<8)/*+(rando()&1023)-512*/,((yy+NM->BornPtY[j-1])<<8)/*+(rando()&1023)-512*/,16,2);
			//if(OBJ->DstX<=0){
			//	j--;
			...
			if(OBJ->DstX>0){
				...
				if(INS!=0xFFFF){
					...
					if(rk<0xFE&&OB->newMons->Peasant){
						...
						OB->NewMonsterSmartSendTo((dx/*OBJ->DstX+((rando()-16384)>>5)*/)>>4,(dy/*OBJ->DstY+((rando()-16384)>>5)*/)>>4,0,0,128,2);
					};
				};
			};
			GotoFinePosition(OB);;
