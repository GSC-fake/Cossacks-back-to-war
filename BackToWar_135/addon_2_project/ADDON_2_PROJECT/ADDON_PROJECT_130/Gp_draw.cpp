...
void CheckGP(){
	for(int i=0;i<GPS.NGP;i++)if(GPS.GPH[i]){
...
void TestGP(){
	...
	GPS.ShowGPLayers(mapx*4+270,mapy+164,442,4,1,0xFFFF&(~512));
...
bool CheckGP_Inside(int FileIndex,int SprIndex,int dx,int dy){
	if(!(FileIndex<GPS.NGP&&(SprIndex&4095)<GPS.GPNFrames[FileIndex]))return false;
