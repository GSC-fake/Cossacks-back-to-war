...
bucketset_t NewBucketSet(void)
{
	...
	set = (bucketset_t)malloc(sizeof (struct bucketset_s));
	assert(set);
...
char *DumpBucketSet(bucketset_t set)
{
	...
	if (set == NULL)
		set = g_buckets;
	assert(set);
	data.data = malloc(128); //alloc an initial buffer
	data.data[0] = 0;
	data.len = 0;
	data.maxlen = 128;
...
void *BucketConcat(bucketset_t set, char *name, void *value)
{
	...
	s = DoGet(pbucket);
	temp = malloc(strlen(s) + strlen(value) + 1);
	strcpy(temp,s);
	strcat(temp, value);
...
static void DumpMap(void *elem, void *clientData)
{
	...
	if (data->maxlen <= minlen) 
	{
		if (data->maxlen == 0)
			...
			data->maxlen *= 2;
		data->data = realloc(data->data, data->maxlen);
...
static void *DoSet(bucket_t *pbucket, void *value)
{
	...
	else if (pbucket->type == bt_string)
	{
		if (pbucket->vals.sval != NULL)
			free(pbucket->vals.sval);
		pbucket->vals.sval = (value == NULL ? NULL : stripchars(_strdup((char *)value)));
	}
	return DoGet(pbucket);
}
...
static bucket_t *DoFind(bucketset_t set, char *name)
{
	...
	tbucket.name = name;
	return TableLookup(set->buckets,&tbucket);
}
