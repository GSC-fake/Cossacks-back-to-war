...
void SaveAI(SaveBuf* SB){
	...
	//safe places for grenaders
	N=0;
	for(i=0;i<SafeLX*SafeLX;i++)if(SCINF[i])N++;
	xBlockWrite(SB,&N,2);
	for(i=0;i<SafeLX*SafeLX;i++)if(SCINF[i]){
		xBlockWrite(SB,&i,4);
...
void LoadAI(SaveBuf* SB){
	...
	xBlockRead(SB,&chk,4);
	assert(chk==123456);
	xBlockRead(SB,TowsID,sizeof TowsID);
	xBlockRead(SB,TowsSN,sizeof TowsSN);
	xBlockRead(SB,TowsMaxR,sizeof TowsMaxR);
	xBlockRead(SB,TowDanger,sizeof TowDanger);
	xBlockRead(SB,&CurTow,sizeof CurTow);
	xBlockRead(SB,&CurR,sizeof CurR);
	xBlockRead(SB,&CurXip,sizeof CurXip);
	xBlockRead(SB,&MaxTowR,sizeof MaxTowR);
	xBlockRead(SB,InflMap,sizeof InflMap);
	xBlockRead(SB,InfoMap,sizeof InfoMap);
	xBlockRead(SB,&NMines,2);
	xBlockRead(SB,&MaxMine,2);
	if(MaxMine){
		MineList=new word[MaxMine];
		xBlockRead(SB,MineList,2*NMines);
	};
	//enemy walls
	xBlockRead(SB,&NEnWalls,4);
	if(NEnWalls){
		xBlockRead(SB,WallsX,NEnWalls<<1);
		xBlockRead(SB,WallsY,NEnWalls<<1);
	};
	//danger objects
	xBlockRead(SB,&NDINF,4);
	if(NDINF){
		xBlockRead(SB,DINF,NDINF*sizeof DangerInfo);
	};
	//safe places for grenaders
	word N=0;
	xBlockRead(SB,&N,2);
	...
	for(int i=0;i<N;i++){
		int j;
		xBlockRead(SB,&j,4);
		SCINF[j]=new SafeCellInfo[64];
		xBlockRead(SB,SCINF[j],64*sizeof SafeCellInfo);
	};
	*/
	xBlockRead(SB,&chk,4);
	assert(chk==123457);
...
void PreSaveGame(SaveBuf* SB,char* Messtr,int ID){
	...
	//------------------
	for(i=0;i<7;i++)if(PINFO[i].name[0]){
...
void SaveGame(char* Name,char* Messtr,int ID){
	...
	char ttt[128];
	strcpy(ttt,Name);
	if(!strstr(ttt,".sav"))strcat(ttt,".sav");
...
SFLB_DLLEXPORT SFLB_PreLoadGame(SaveBuf* SB,bool LoadNation){
	...
	if(tt==2)xBlockRead(SB,&SaveState,1);
...
void SetupArrays(){
	...
	ARRSZ+=szz;
	memset(MCount,0,VAL_MAXCIOFS);
	MRef=new word[VAL_MAXCIOFS];
	memset(MRef,0xFF,VAL_MAXCIOFS*2);
	szz=VAL_MAXCIOFS*2;
	ARRSZ+=szz;
	//NMsList=new word[VAL_MAXCIOFS*MAXINCELL];
	//szz=VAL_MAXCIOFS*MAXINCELL*2;
	//ARRSZ+=szz;
	//memset(NMsList,0,VAL_MAXCIOFS*MAXINCELL*2);
