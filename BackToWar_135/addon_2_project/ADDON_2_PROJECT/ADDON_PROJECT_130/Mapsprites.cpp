...
//--------------Local 3D heights-------------//
/*
byte* L3HIMap[L3DX*L3DY];
void InitL3(){
	memset(L3HIMap,0,sizeof L3HIMap);
};
void ClearL3(){
	int n=sizeof L3HIMap>>2;
	for(int i=0;i<n;i++)if(L3HIMap[i]){
		free(L3HIMap[i]);
		L3HIMap[i]=NULL;
	};
};
void SetL3Point(int x,int y,byte H){
	if(x>=0&&y>=0&&x<L3MAX&&y<L3MAX){
		int ofs=(x>>4)+((y>>4)<<L3SH);
		if(!L3HIMap[ofs]){
			L3HIMap[ofs]=new byte[256];
			memset(L3HIMap[ofs],0,256);
		};
		L3HIMap[ofs][(x&15)+((y&15)<<4)]=H;
	};
};
...
int OneObject::TakeResource(int x,int y,byte ResID,int Prio,byte OrdType){
	...
	if(DObj==INITBEST)return INITBEST;
	RStage=0;
