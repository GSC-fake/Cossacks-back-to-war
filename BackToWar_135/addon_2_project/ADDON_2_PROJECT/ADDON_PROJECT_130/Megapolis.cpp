	...
	if(ofst>=0&&ofst<MaxTop){
		...
		if(!OurZone){
			if(!OR1){
				...
				if(Cur!=0xFFFF){
					OB->NewMonsterSendTo(int(TopMap[Cur].x)<<10,int(TopMap[Cur].y)<<10,0,0);
					return true;
				};
			};
		}else{
	...
	for(i=0;i<MAXOBJECT;i++){
		...
		if(OB&&OB->NNUM==NI&&(OB->Hidden||!OB->Sdoxlo)){
			...
			if((NM->Building&&OB->Stage>=ACR->ProduceStages)||(!NM->Building)){
				...
				NewMonster* NM=OB->newMons;
				if(NM->Farm)Nat->NFarms+=NM->NInFarm;
	...
	if(w){
		if(OBJ->delay){
			...
			if(Top1!=0xFFFF&&Top2!=0xFFFF){
				if(Top1==Top2||MotionLinks[Top1*NAreas+Top2]==Top2){
		...
		if(SearchGreAttObject(&x0,&y0,40,OBJ->NNUM)){
			...
			if(AAR){
				...
				if(HimTop!=0xFFFF){
					...
					if(MyTop!=0xFFFF){
						int NextTop=HimTop;
						if(MyTop!=HimTop)NextTop=MotionLinks[MyTop*NAreas+HimTop];
						if(NextTop!=0xFFFF){
							if(NextTop==HimTop){
								//connect
								OBJ->ClearOrders();
...
bool CheckWallForKilling(OneObject* OB){
	if(OB->Wall&&OB->WallX>0&&OB->WallY>0&&OB->WallX<TopLx-1&&OB->WallY<TopLy-1){
		...
		int TPL=TopRef[LI-1];
		if(TPL==0xFFFF)TPL=TopRef[LI-2];
		int TPR=TopRef[LI+1];
		if(TPR==0xFFFF)TPR=TopRef[LI+2];
		int TPU=TopRef[LI-TopLx];
		int TPD=TopRef[LI+TopLx];
		if(TPD==0xFFFF)TPD=TopRef[LI+TopLx+TopLx];

		if((TPL<NAreas||TPR<NAreas)&&(TPU<NAreas||TPD<NAreas))return true;
		if((TPL==TPR&&TPL<NAreas)||(TPU==TPD&&TPU<NAreas))return false;
		if(TPL<NAreas&&TPR<NAreas){
			if(LinksDist[TPL*NAreas+TPR]<35)return false;
		};
		if(TPU<NAreas&&TPD<NAreas){
			if(LinksDist[TPU*NAreas+TPD]<35)return false;
		};
		return true;
	}else return false;
};
int GetBestVictimForArchers(byte NI,int x,int y,int R,int MyTop){
	if(MyTop<0||MyTop>=NAreas)return 0xFFFF;
	char ARRAY[64*64];
	word BESTID[64*64];
