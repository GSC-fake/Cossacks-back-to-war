...
bool CreateInsideList(IconSet* IS,byte NI){
	...
	if(Nsel){
		...
		if(OneType&&!(OB->newMons->Capture||OB->NewBuilding||OB->Transport||OB->Wall||OB->newMons->Usage==PeasantID)){
			if(Guards&&!Usual){
...
void TakeResourceFromSpriteLink(OneObject* OBJ){
	...
	byte RTP=OBJ->RType;
	if(RTP==GoldID)NInGold[NT]+=OBJ->NInside;
	else if(RTP==IronID)NInIron[NT]+=OBJ->NInside;
	else if(RTP==CoalID)NInCoal[NT]+=OBJ->NInside;
	IRES+=work;
    OBJ->RAmount+=work;
	if(IRES>1000){
