...
bool StartIGame(bool SINGLE){
	...
	int HostID=0;
	for(i=0;i<NPlayers;i++)if(PINFO[i].Host)HostID=i;
	...
	DWORD SUM=0;
	for(i=0;i<42+8+8+1;i++)SUM+=BUF[i];
	BUF[42+8+8+1]=SUM;
	FreePDatas();
	...
	do{
		do{
			...
			if(yyy>80){
				...
				if(!MiniActive)DrawStdBar(xc-150,yc,xc+150,yc+30+NPlayers*18+30);
				int y0=yc+12;
...
void SyncroDoctor(){
	...
	if(tc-LastSynTime>30000||tc<LastSynTime){
		...
		if(UseGSC_Login&&(GetTickCount()-PREVSYNC>20000)){
...
bool CheckPingsReady(){
	for(int i=0;i<NPlayers;i++)if(PINFO[i].PlayerID!=MyDPID&&PSUMM.CheckPlayer(PINFO[i].PlayerID)<12)return false;
	return true;
};
bool CheckExistConn(){
