...
void CmdEndGame(byte NI,byte state,byte cause){
	...
	if(UseGSC_Login&&!PlayGameMode){
		for(int i=0;i<NPlayers;i++)if(PINFO[i].ColorID==NI){
			ReportIt("CmdEndGame(%s,%d)",PINFO[i].name,state);
...
void __EndGame(byte NI,byte state){
	...
	if(NI==MyNation){
		...
		WaitState=1;
		for(i=0;i<NPlayers;i++)if(PINFO[i].ColorID==NI){
	...
	if(UseGSC_Login&&!PlayGameMode){
		for(int i=0;i<NPlayers;i++)if(PINFO[i].ColorID==NI){
			ReportIt("__EndGame(%s,%d)",PINFO[i].name,state0);
...
void CreateGoodSelection(byte NI,word xx,word yy,word xx1,word yy1,CHOBJ* FN,int NN,bool Addon){
	...
	int Olds=0;
	if(Addon)Olds=Nsel;
	word nnm=GoodSelectNewMonsters(NI,xx,yy,xx1,yy1,NULL,NULL,false,FN,NN,ULIMIT);
	if(Olds+ns+nnm){
...
void MakeReformation(byte NI,word BrigadeID,byte FormType){
	...
	if(BR->Enabled&&BR->WarType){
		...
		if(j<SG->NForms){
			...
			for(q=2;q<RealNM;q++)BR->MembSN[q]=Group[BR->Memb[q]]->Serial;
			BR->WarType=WT;
...
void DipCheck(){
	return;
	if(!PrevCallTime)PrevCallTime=GetTickCount();
	if(GetTickCount()-PrevCallTime<10000)return;
	PrevCallTime=GetTickCount();
	int NLOCK;
	for(int i=0;i<7;i++){
		int NDIP=0;
		int N=NtNUnits[i];
		...
		for(int j=0;j<N;j++){
			...
			if(MID!=0xFFFF){
				...
				if(OB&&OB->NewBuilding){
					...
					if(GO->NLockUnits&&GO->LockID==OB->NIndex&&NLOCK<32){
						if(CITY[i].UnitAmount[OB->NIndex]>GO->NLockUnits){
							LOCKMID[NLOCK]=OB->Index;
							NLOCK++;
						};
					};
				};
		...
		if(NLOCK){
			CmdFreeSelected(i);
			CmdSelUnitsSet(i,LOCKMID,NLOCK);
			CmdDie(i);
		};
	};
};