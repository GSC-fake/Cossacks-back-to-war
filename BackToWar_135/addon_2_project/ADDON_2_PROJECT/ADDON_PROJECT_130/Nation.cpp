...
bool CheckAttAbility(OneObject* OB,word Patient){
	if(OB&&Patient!=0xFFFF){
		...
		if(OB->newMons->Priest){
			...
			if(EN->Life>=EN->Ref.General->MoreCharacter->Life)return false;
		}else{
			if(EN->NMask&OB->NMask)return false;
		};
...
void OneObject::Die(){
	...
	addname(Index);
	if(!(NewBuilding||LockType||newMons->Artilery))RealDir+=(rando()>>8)-64;
		...
		for(int i=0;i<MAXOBJECT;i++){
			...
			if(OB&&OB->Wall){
				...
				if(!WCL){
					DelObject(OB);
					if(OB->Index<8192){
						if(OB->newMons->NBars)Delete3DBar(OB->Index);
						Group[OB->Index]=NULL;
					};
				};
			};
...
void OneObject::MakeDamage(int Fundam,int Persist,OneObject* Sender,byte AttType){
	...
	if(Sender&&!Zombi)AttackObj(Sender->Index,1);
};
