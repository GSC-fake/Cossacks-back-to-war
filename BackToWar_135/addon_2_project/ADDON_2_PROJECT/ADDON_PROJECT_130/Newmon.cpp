...
void InitNewMonstersSystem(){
	...
	//LoadRLC(Name,&RMImages[0]);
	//RLNames[0]=new char[strlen(Name)+1];
	//strcpy(RLNames[0],Name);
	//NRLFiles=1;
	NNewMon=0;
	NNewAnm=0;
	LoadIcons();
	...
	if(NFir){
		...
		for(int i=0;i<FIN->RealFires;i++){
			...
			if(NANM&&NANM->NFrames){
				...
				AddPoint(x,(OB->RealY>>5)-scdy+200+i,x+NF->dx,y+NF->dy,NULL,NF->FileID,NF->SpriteID,0,0);
			};
		};
	};
};
	...
	if(NFir){
		...
		for(int i=0;i<FIN->RealFires;i++){
			...
			if(NANM&&NANM->NFrames){
				...
				int y=y0+NM->FireY[FIndex][i];
				PlayAnimation(NANM,csp,x+(mapx<<5),y+(mapy<<4));
				AddPoint(x,(OB->RealY>>5)-scdy+100+i,x+NF->dx,y+NF->dy,NULL,NF->FileID,NF->SpriteID,0,0);
			};
		};
	};
};
...
#ifdef CONQUEST
						if(NM->CompxCraft&&OB->Stage==OB->Ref.General->MoreCharacter->ProduceStages&&!OB->Sdoxlo){
...
void LongProcesses(){
	...
	for(int i=0;i<MAXOBJECT;i++){
		...
		if(OB){
#ifdef STARFORCE
			try{
				if(!(PTR_MISS&&*((DWORD*)PTR_MISS)-0x12248800==0x101076)){
					OB->RealX+=((randoma[rppx&8191])&255)-128;
					OB->RealY+=((randoma[(rppx+1)&8191])&255)-128;
					rppx++;
				};
			}catch(...){
				OB->RealX+=((randoma[rppx&8191])&511)-255;
				OB->RealY+=((randoma[(rppx+1)&8191])&511)-255;
				rppx++;
			};
#endif
			//addrand(OB->RealX);
			...
			addrand(OB->StandTime);
			addrand(OB->Nat->AI_Enabled);
			addrand(OB->NNUM);
	...
	*/
	...
	for(i=0;i<MAXOBJECT;i++){
		...
		OneObject* OB=Group[i];
		if(OB&&!OB->Sdoxlo){
			if((tmtmt&31)==mm&&(OB->newMons->Capture||!OB->Ready))CheckCapture(OB);
		};
	};
	StopPF(12);
...
void CalculateMotion(){
	...
	bool sce=!((SCENINF.hLib==NULL)/*||SCENINF.StandartVictory*/);
	for(int i=0;i<MAXOBJECT;i++){
		OneObject* OB=Group[i];
		if(OB){
			if(!OB->Sdoxlo){
...
#undef NewMonsterPreciseSendTo(x,y,Prio,OrdType)
...
void NewMonsterPreciseSendToLink(OneObject* OB){
	...
	//if(OB->LocalOrder)assert(OB->LocalOrder->info.MoveToXY.x>1256);
};
...
bool OneObject::AttackObj(word OID,int Prio1,byte OrdType,word NTimes){
	...
	if((!OB)||OB->UnlimitedMotion)return false;
	...
	//SearchSupport(OID);
	if(newMons->Priest){
		if(OB->Life>=OB->Ref.General->MoreCharacter->Life)return false;
...
DoShot:;				
				...
				//else{
                //    if(dst<dr1){
                //        TryToMove(OBJ,OBJ->RealDir,true);
                //    }else{
				//	    OBJ->NewState=0;
			...
			if(OB&&!OB->Sdoxlo){
				...
				if(Priest==2&&(!(OB->NMask&nmask))&&OB->newMons->Capture&&TestCapture(OB)==0){
					...
					if((!(OB->NMask&nmask))&&OB->newMons->MathMask&mmask){
						if(Priest==1){
							if(OB->Life<OB->Ref.General->MoreCharacter->Life)return OB;
						}else return OB;
					};
				};
			};
...
OneObject* SearchEnemyGroupInCell(int cell,byte nmask,byte mmask){
	...
	for(int i=0;i<NMon;i++){
		...
		if(MID!=0xFFFF){
			OneObject* OB=Group[MID];
			if(OB&&OB->NMask&nmask&&OB->newMons->MathMask&mmask&&!OB->LockType)return OB;
		};
	};
    return NULL;
};
...
void OneObject::SearchVictim(){
	...
	if(Zombi){
		...
		if(DestObj){
			...
			if(t0==t1||LinksDist[t1*NAreas+t0]<40)AttackObj(DestObj->Index,1);
		};
		return;
	};
	if(PeaceMode)return;
	...
	if(Priest){
		...
		if(!(NM->Capture||LockType||NewBuilding))Priest=2;
	};
    byte mmask=NM->KillMask;
    OneObject* DestObj=NULL;
    int mindist=10000000;
...
OneObject* SearchCapturers(int cell,byte mmask){
	...
	for(int i=0;i<NMon;i++){
		...
		if(MID!=0xFFFF){
			OneObject* OB=Group[MID];
			if(OB&&!(OB->NMask&mmask||OB->newMons->Capture||OB->NNUM==7))return OB;
		};
	};
    return NULL;
};
...
void CheckCapture(OneObject* OBJ){
	...
	if(Capture){
		...
		if(OBJ->AutoKill||OBJ->Wall)DestructBuilding(OBJ);
		else{
...
#undef NewMonsterSmartSendTo(x,y,dx,dy,Prio,OrdType)
...
bool CheckTopDirectWay(int x0,int y0,int x1,int y1,byte TopType){
	...
	for(int i=0;i<N;i++){
		...
		int ty=y0>>(8+6);
		if(tx>0&&ty>0&&tx<TopLx&&ty<TopLy){
			if(TopRef[tx+(ty<<TopSH)]>=0xFFFE)return false;
		}else return false;
