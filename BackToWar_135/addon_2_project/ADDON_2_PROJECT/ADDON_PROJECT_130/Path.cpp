...
void MotionField::CreateField(int fLx,int fLy){
	RealLx=(fLx+64)>>3;
	RealLy=(fLy+64);
	Dx=32;
	Dy=32;
...
void MotionField::DrawPoint(int x,int y,bool Color){
	if(x>=0&&y>=0&&x<Lx&&y<Ly){
		int xx=(x+Dx)>>3;
		int dx=(x+Dx)&7;
		if(Color){
			Field[(y+Dy)*RealLx+xx]|=1<<dx;
		}else{
			Field[(y+Dy)*RealLx+xx]&=~(1<<dx);
		};
	};
};
void MotionField::DrawBar(int x,int y,int BLx,int BLy,bool Color){
	if(x>=Lx||y>=Ly)return;
	if(x<0){
		Lx+=x;
		x=0;
	...
	if(x+BLx>Lx)BLx=Lx-x;
	if(y+BLy>Ly)BLx=Ly-y;
	if(BLx<=0||BLy<=0)return;
	//ready to draw
	int DXL=x&7;
	int DXR=(x+BLx)&7;
	int L1=(BLx-8+DXL-DXR)>>3;
	if(L1<0){
		int ofst=(y+Dy)*RealLx+(x>>3);
		byte ms=(((((0xFF)>>DXL)<<DXL)<<DXR)>>DXR);
		for(int yy=0;yy<BLy;yy++){
			Field[ofst]|=ms;
			ofst+=RealLx;
		};
	}else{
		...
		for(int  yy=0;yy<BLy;yy++){
			Field[ofst]|=msL;
			Field[ofst+L1+1]|=msR;
			for(int i=0;i<L1;i++)Field[ofst+i]=0xFF;
			ofst+=L1;
		};
	};
};
void MotionField::DrawBrush(int x,int y,MotionBrush* Brush,bool Color){
	if(x<-31||x>=Dx+Lx||y<-31||y>=Dy+Ly||Brush->Ly<=0)return;
	int ofst=int(Field)+((x+Dx)>>3)+(y+Dy)*RealLx;
	int BOFS=int(Brush->Data);
	int BLY=Brush->Ly;
	int RLX=RealLx;
	int Shif=(x+Dx)&7;
	__asm{
...
bool MotionField::CheckBrush(int x,int y,MotionBrush* Brush){
	if(x<-31||x>=Dx+Lx||y<-31||y>=Dy+Ly||Brush->Ly<=0)return true;
	int ofst=int(Field)+((x+Dx)>>3)+(y+Dy)*RealLx;
	int BOFS=int(Brush->Data);
	int BLY=Brush->Ly;
	int RLX=RealLx;
...
int  MotionField::FindWay(int x,int y,int x1,int y1,MotionBrush* Brush,short** WayX,short** WayY,int Step,int Border){
...
bool FindBestPosition(OneObject* OB,int* xd,int* yd,int R0){
	...
	if(!OB)return false;
	if(!MFI->CheckBar((*xd)-1,(*yd)-1,OB->Lx+2,OB->Lx+2))return true;
	int Lx=OB->Lx+2;
	int R1=R0;
	int bx=*xd;
	int by=*yd;
	...
	if(Rtx!=x1||Rty!=y1){
		...
		if(Rpp<MaxP-9){
			if(RightPrefer){
				...
				for(int i=0;i<RVisPos&&RVIS[i]<Pps;i++);
				...
				//Rvp=Lvp;
				//search for direct vision point
				for(int i=0;i<LVisPos&&LVIS[i]<Pps;i++);
