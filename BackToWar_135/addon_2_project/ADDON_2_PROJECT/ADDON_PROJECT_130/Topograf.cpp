...
void CreateAreas(){
	...
	*/
	...
	for(i=0;i<NAreas;i++)TopRef[TopMap[i].x+int(TopMap[i].y)*TopLx]=i;
	bool Change=false;
	...
	for(i=0;i<NAreas;i++)if(TopMap[i].NMines){
...
void CreateAreas(){
	...
	NAreas=0;
	//creating ref. map
	memset(TopRef,0xFFFF,sizeof TopRef);
	int mmx=msx>>1;
	int mmy=msy>>1;
	for(int x=0;x<mmx;x++)
		for(int y=0;y<mmy;y++){
			int xxx=x<<2;
			int yyy=y<<2;
			if(!MFIELDS->CheckBar(xxx,yyy,4,4))TopRef[x+y*TopLx]=0xFFFE;
			else{
				//accurate check
				if( ( (!CheckPt(xxx+1,yyy))||(!CheckPt(xxx+2,yyy+1))||(!CheckPt(xxx+3,yyy+2)) )||
					( (!CheckPt(xxx,yyy+1))||(!CheckPt(xxx+1,yyy+2))||(!CheckPt(xxx+2,yyy+3)) )||
	...
	for(int i=0;i<MAXSPR;i++){
		...
		SprGroup* SG=OS->SG;
		ObjCharacter* OC=&SG->ObjChar[OS->SGIndex];
		if(OC->IntResType<8)AddArea(OS->x>>6,OS->y>>6,true);
	};
	do{
		int x=(int(rando())*(mmx-4))>>15;
		int y=(int(rando())*(mmy-4))>>15;
	...
	for(i=1;i<RRad&&Change;i++){
		...
		for(int j=0;j<NAreas;j++){
			...
			for(int k=0;k<N;k++){
				...
				if(x>0&&y>0&&x<mmx&&y<mmy){
					int ofst=x+y*TopLx;
					word ZZ=TempTop[ofst	  ];
					word ZU=TempTop[ofst-TopLx];
					word ZD=TempTop[ofst+TopLx];
					word ZL=TempTop[ofst-1    ];
					word ZR=TempTop[ofst+1    ];
					if(ZZ==0xFFFE){
						if(ZU==j||ZD==j||ZL==j||ZR==j||
							TempTop[ofst-TopLx-1]==j||
							TempTop[ofst-TopLx+1]==j||
							TempTop[ofst+TopLx-1]==j||
							TempTop[ofst+TopLx+1]==j){
							TopRef[ofst]=j;
...
NoDraw:	inc		edi
	...
	if(CurZone<0xFFFE){
		for(int i=0;i<NAreas;i++){
			if(i!=CurZone){
				int j=MotionLinks[i*NAreas+CurZone];
				if(j!=0xFFFF){
					Area* AR=TopMap+i;
					int xx=int(AR->x)-x0;
					int yy=int(AR->y)-y0;
...
void RemakeGroupOfAreas(word* Grp,int Na){
	...
	for(int i=0;i<Na;i++){
		...
		int k=AR->NLinks;
		for(int j=0;j<k;j++){
			int ofst=int(AR->Link[j+j])+ID*NAreas;
...
void DrawForces(int xs,int ys,int x0,int y0,int Lx,int Ly){
	...
	for(int i=0;i<NAreas;i++){
		...
		ShowString(TopMap[i].x+xs-3,TopMap[i].y+ys-3,cc,&fn10);
		int w=NearWater[i];
		if(w!=0xFFFF){
...
void RemakeGroupOfAreas(word* Grp,int Na){
		...
		for(int p=0;p<NAreas;p++){
			...
			if(L_QP<0xFFFE){
				...
				for(int t=0;t<NBadLinks;t++){
					...
					int L_CD=LinksDist[C+D*NAreas];
					int L_QC=q==C?0:LinksDist[ML_Q+C];
					int L_QD=q==D?0:LinksDist[ML_Q+D];
					int L_CP=p==C?0:LinksDist[p+C*NAreas];
					int L_DP=p==D?0:LinksDist[p+D*NAreas];
					int D_L=L_QP-L_CD;
	...
	*/
	...
	for(q=0;q<NN;q++)if(LinksDist[q]==0xFFFF)MotionLinks[q]=0xFFFF;
	//Stop=1;
	free(OLN);
};
void OnePartIterationNew(int Nitr){
	...
	if(CurIStart==NAreas-1)CurIStart=0;
};
