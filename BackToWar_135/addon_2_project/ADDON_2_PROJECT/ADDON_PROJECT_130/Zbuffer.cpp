...
void ShowZBuffer(){
	...
	for(i=0;i<NHIITEMS;i++)ShowZElement(ZCashe+HILAYER[i]);
	if(!CINFMOD)return;
	for(i=DYZBuf;i<1600;i++){
		if(ZBFCounter[i]){
			int n=ZBFCounter[i];
			int pos=i<<ZSHF;
			for(int j=0;j<n;j++){
