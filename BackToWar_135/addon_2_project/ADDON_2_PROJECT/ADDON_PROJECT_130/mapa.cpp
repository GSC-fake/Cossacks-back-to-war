...
int SortPlayers(byte* Res,int* par){
	...
	for(int q=0;q<7;q++){
		if(NtNUnits[q]){
			SC.Uids[np]=q;
			if(con==1)SC.Parms[np]=-(SumSquare[q]*1000+NatSquare[q]);
			else SC.Parms[np]=-(SumAccount[q]*10000+CITY[q].Account);
			np++;
		};
	};
	SC.NUids=np;
...
void GFieldShow(){
	...
	/*
	if(EditMapMode&&NSL[MyNation]&&!NoText){
        word MID=Selm[MyNation][0];
        if(MID!=0xFFFF){
    		OneObject* OBB=Group[MID];
	    	if(OBB&&OBB->PathX){
			    for(int i=0;i<OBB->NIPoints;i++){
				   	int xx=smapx+((int(OBB->PathX[i])-(mapx<<1))<<4);
				    int yy=smapy+(mul3(int(OBB->PathY[i])-(mapy<<1))<<2);
    				yy-=GetHeight(OBB->PathX[i]<<4,OBB->PathY[i]<<4);
	    			char ggx[16];
		    		sprintf(ggx,"%d",i);
			    	ShowString(xx,yy,ggx,&fn8);
                };
			};
		};
	};
                ...
                if(NM->Wall){
    		        ...
    		        if(GetRealCoords(mouseX,mouseY,&xxx,&yyy)){
	    		        ...
	    		        if(CheckSmartCreationAbility(NatRefTBL[MyNation],NM,&xx3,&yy3)!=-1){
							x0=(xx3>>4)-(mapx<<5)+NM->PicDx;
							y0=(yy3>>5)-(mapy<<4)+NM->PicDy-GetHeight(xx3>>4,yy3>>4);
							if(DN->LineInfo){
								for(int p=0;p<DN->NFrames;p++){
									NewFrame* OF=&DN->Frames[p];
									GPS.ShowGPDarkN(x0,y0,OF->FileID,OF->SpriteID,0,NNN);
								};
							}else{
	    						if(UP->Enabled){
	    							for(int i=0;i<UP->Parts;i++){
	    								NewFrame* NF=&UP->Frames[i];
										GPS.ShowGPDarkN(x0+i*UP->PartSize,y0,NF->FileID,NF->SpriteID,0,NNN);
	    							};
	    						};
	    						if(DN->Enabled){
	    							for(int i=0;i<DN->Parts;i++){
	    								NewFrame* NF=&DN->Frames[i];
										GPS.ShowGPDarkN(x0+i*DN->PartSize,y0,NF->FileID,NF->SpriteID,0,NNN);
	    							};
	    						};
							};
							if(WR->Enabled){
	    						NewFrame* NF=WR->Frames;
								GPS.ShowGPDarkN(x0+NF->dx,y0+NF->dy,NF->FileID,NF->SpriteID,0,NNN);
							};
							...
							if(NM->Port){
								...
								if(BUANM){
									...
									px+=NF->dx;
									py+=NF->dy;
									GPS.ShowGPDarkN(px-LL-LL,py,NF->FileID,NF->SpriteID,0,NNN);
									GPS.ShowGPDarkN(px+LL+LL,py,NF->FileID,NF->SpriteID,0,NNN);
									GPS.ShowGPDarkN(px,py-LL,NF->FileID,NF->SpriteID,0,NNN);
									GPS.ShowGPDarkN(px,py+LL,NF->FileID,NF->SpriteID,0,NNN);
								};
							};
							if(AddGP!=-1){
								for(int i=0;i<MaxAStages;i++){
									if(sprs[i]!=-1){
										GPS.ShowGPDarkN(x0,y0,AddGP,sprs[i],0,NNN);
									};
								};
							};
	    		        }else{
	...
	*/
	...
	ScenaryLights();
	if(SHOWSLIDE&&FogMode&&BaloonState!=2&&(!NATIONS[NatRefTBL[MyNation]].Vision)&&!NoText){
		time0=GetRealTime();
...
void GMiniShow(){
	...
	PreviewBrig=-1;
	if(FogMode&&(!NATIONS[NatRefTBL[MyNation]].Vision)&&BaloonState!=2)DrawMiniFog();
	HandleGeology();
	...
	*/
		...
		for(int i=0;i<N;i++){
			...
			if(MID!=0xFFFF){
				...
				if(ME&&ME->Serial==sns[i]&&!ME->Sdoxlo){
					...
					if(NM->Peasant)CanWork=1;
					if(!NM->Priest)MTMASK|=NM->KillMask;
					if(!(ME->NewBuilding||ME->LockType))CanEnter=1;
					if(NM->CanShoot)CanShoot=1;
				};
			};
		};
		if(GoAndAttackMode)curptr=1;
		if(SetDestMode)curptr=8;
		if(AttGrMode)curptr=AttGrPtr;
	};
	...
	if(SpecCmd==112){
		if(LastActionX!=0){
			...
			mapy=(LastActionY>>5)-(smaply>>1)-(GetHeight(LastActionX,LastActionY)>>4);
			if(mapx<=0)mapx=1;
			if(mapy<=0)mapy=1;
	...
	if(cooren){
		...
		if(HeightEditMode==1||HeightEditMode==10||HeightEditMode==11||HeightEditMode==12||HeightEditMode==13){//edit height mode
			bool LockKey=0;
			if(Lpressed){
		...
		if(ImNSL[MyNation]>0&&!mini){
			...
			if(MTMASK||CanAttBLD||CanCapt){
				...
				if(Att!=0xFFFF&&!GoAndAttackMode){
					...
					if(AOB){
						if(AOB->newMons->CanBeKilledInside&&AOB->NInside&&CanShoot)curptr=1;
						if(AOB->newMons->MathMask&MTMASK)curptr=1;
						else{
							if(AOB->NewBuilding||AOB->Wall||AOB->LockType){
								if(CanAttBLD)curptr=1;
							}
						};
						if(AOB->newMons->Capture&&CanCapt&&TestCapture(AOB)!=-1)curptr=2;
						if(GetKeyState(VK_CONTROL)&0x8000){
							if(AOB->newMons->CanBeCapturedWhenFree)curptr=8;
							else curptr=8;
						};
					};
				};
			...
			if(CanWork){
				if(Friend!=0xFFFF){
					...
					if(FR&&(FR->NewBuilding)&&!FR->Sdoxlo){
						if(FR->Ready&&FR->Ref.General->MoreCharacter->MaxInside+FR->AddInside>FR->NInside)curptr=4;
						if(FR->Life<FR->Ref.General->MoreCharacter->Life)curptr=3;
					};
				};
	...
	if(!SCENINF.hLib){
		for(int k=1;k<7;k++){
			...
			if(nidx!=-1){//strcmp(PINFO[k].name,NoPlayer)){
				...
				Nation* NT=NATIONS+k;
				int MaxA=-1;
				char* NtID=GlobalAI.Ai[nidx].NationID;
				int Land=LAND[k];
				for(int w=0;w<10;w++){
				...
				NT->CITY->StartRes=STRS[k];//CMGRP1[2]->CurLine;
				NT->CITY->ResOnMap=RONM[k];//CMGRP1[3]->CurLine;
				NT->CITY->Difficulty=DIFF[k];
				NT->CITY->LandType=LAND[k];//RMP.STY[CMGRP1[0]->CurLine].AI_Style;
				NT->AI_Enabled=true;
				NT->NMask=0x7E;
...
void UnitInfo::Create(int ID){
	...
	//CurSHL
	for(i=0;i<9;i++)if(NM->Protection[i])CurSHL[i+1]=NM->Protection[i];
	Shield=NM->Shield;
		...
		for(int j=0;j<N;j++){
			if(NATIONS->UnitsIDS[i][j]==UI_Current){
				if(j<N-1){
					...
					if(UI_Current==0xFFFF)UI_Current=NATIONS->UnitsIDS[i][j+2];
					return true;
				}else{
					for(int k=0;k<NNations;k++){
						if(NATIONS->NUnits[(i+k+1)%NNations]){
							UI_Current=NATIONS->UnitsIDS[(i+k+1)%NNations][0];
							if(UI_Current==0xFFFF)UI_Current=NATIONS->UnitsIDS[(i+k+1)%NNations][1];
							return true;
						};
					};
				};
...
int CreateUNITINF_UNIT(int ID){
	...
	if(UINF.Picture)UNITINF.addBPXView(NULL,dx,dy,UINF.Lx,UINF.Ly,1,1,1,UINF.Picture,NULL);
	GPPicture* Panel;
	...
	RLCFont* FN5=&fn10;
	UNITINF.addTextButton(NULL,4,-24,UINF.Name,&WhiteFont,&WhiteFont,&WhiteFont,0);
	UNITINF.HintX=-1;
	UNITINF.HintY=280;
	...
	case 0://Unit
		dxx=2;
		dyy=1+UNI_LINEDY2;
		Panel=UNITINF.addGPPicture(NULL,0,0,BordGP,19);

		UNITINF.addGPPicture(NULL,115+dxx,11+dyy,BordGP,54);
		General=UNITINF.addGP_TextButton(NULL,115+dxx,11+dyy,BTN_General,BordGP,55,FN1,FN2);

		UNITINF.addGPPicture(NULL,115+dxx,33+dyy,BordGP,54);
		Description=UNITINF.addGP_TextButton(NULL,115+dxx,33+dyy,BTN_Descr,BordGP,55,FN1,FN2);

		UNITINF.addGPPicture(NULL,115+dxx,55+dyy,BordGP,54);
		UnitUpgrades=UNITINF.addGP_TextButton(NULL,115+dxx,55+dyy,BTN_Upgr,BordGP,55,FN1,FN2);

		UNITINF.addGPPicture(NULL,115+dxx,77+dyy,BordGP,54);
		Next=UNITINF.addGP_TextButton(NULL,115+dxx,77+dyy,BTN_Next,BordGP,55,FN1,FN2);

		UNITINF.addGPPicture(NULL,115+dxx,99+dyy,BordGP,54);
		Previous=UNITINF.addGP_TextButton(NULL,115+dxx,99+dyy,BTN_Prev,BordGP,55,FN1,FN2);
		break;
	case 1://Artillery
		dxx=3;
		dyy=-2+UNI_LINEDY2;
		Panel=UNITINF.addGPPicture(NULL,0,0,BordGP,21);
	...
	case 2://Ship
		dxx=3;
		dyy=16+UNI_LINEDY2;
		Panel=UNITINF.addGPPicture(NULL,0,0,BordGP,20);

		UNITINF.addGPPicture(NULL,137+dxx,4+dyy,BordGP,56);
		General=UNITINF.addGP_TextButton(NULL,137+dxx,4+dyy,BTN_General,BordGP,57,FN1,FN2);

		UNITINF.addGPPicture(NULL,137+dxx,26+dyy,BordGP,56);
		Description=UNITINF.addGP_TextButton(NULL,137+dxx,26+dyy,BTN_Descr,BordGP,57,FN1,FN2);

		UNITINF.addGPPicture(NULL,137+dxx,48+dyy,BordGP,56);
		UnitUpgrades=UNITINF.addGP_TextButton(NULL,137+dxx,48+dyy,BTN_Upgr,BordGP,57,FN1,FN2);

		UNITINF.addGPPicture(NULL,137+dxx,70+dyy,BordGP,56);
		Next=UNITINF.addGP_TextButton(NULL,137+dxx,70+dyy,BTN_Next,BordGP,57,FN1,FN2);

		UNITINF.addGPPicture(NULL,137+dxx,93+dyy,BordGP,56);
		Previous=UNITINF.addGP_TextButton(NULL,137+dxx,93+dyy,BTN_Prev,BordGP,57,FN1,FN2);
		break;
	case 3://building
		Panel=UNITINF.addGPPicture(NULL,0,0,BordGP,4);

		UNITINF.addGPPicture(NULL,5,159,BordGP,54);
		Description=UNITINF.addGP_TextButton(NULL,5,159+UNI_LINEDY2,BTN_Descr,BordGP,55,FN1,FN2);

		UNITINF.addGPPicture(NULL,124,159,BordGP,54);
		PriceUpg=UNITINF.addGP_TextButton(NULL,124,159+UNI_LINEDY2,BTN_PriceUpg,BordGP,55,FN1,FN2);

		UNITINF.addGPPicture(NULL,124,182,BordGP,54);
		Next=UNITINF.addGP_TextButton(NULL,124,182+UNI_LINEDY2,BTN_Next,BordGP,55,FN1,FN2);

		UNITINF.addGPPicture(NULL,5,182,BordGP,54);
		Previous=UNITINF.addGP_TextButton(NULL,5,182+UNI_LINEDY2,BTN_Prev,BordGP,55,FN1,FN2);
		break;
	};
	if(General){
	...
	if(PriceUpg){
		...
		if(dd>0){
			...
			VScrollBar* GLSB=UNITINF.addNewGP_VScrollBar(NULL,xx+220,yy0-11,InfY1-UINF.Y1Line,dd,0,3,0);
			GLSB->OnesDy=10;
			UNITINF.addClipper(0,UINF.Y1Line+2,RealLx-1,InfY1);
			GLSB->ScrDy=100;
	...
	if(UnitUpgrades){
		...
		if(UINF.NATT){
			...
			if(NAL){
				...
				for(j=0;j<8;j++)if(UINF.ATTMask&(1<<j)){
					...
					UINF.AddSD(SD,2);
					SD=UNITINF.addGPPicture(NULL,xx+1,yy,4,WeaponIcn[j]);
					UINF.AddSD(SD,2);
					...
					for(int k=0;k<UINF.NATT;k++){
						if(UINF.ATTACK[k].AddValue[j]){
							...
							SD=UNITINF.addTextButton(NULL,xx+62+26*k,yy,cc,FN5,FN5,FN5,1);
							UINF.AddSD(SD,2);
					...
					SD=UNITINF.addTextButton(NULL,xx+31,yy,cc,FN5,FN5,FN5,1);
					UINF.AddSD(SD,2);
					yy+=14;
					CAL++;
				};
		...
		if(UINF.NSHL){
			...
			if(NAL){
				SD=UNITINF.addTextButton(NULL,xx,yy,GetTextByID("UI_ShlUpg"),FN3,FN3,FN3,0);
				UINF.AddSD(SD,2);
				yy+=DY;
				SD=UNITINF.addGPPicture(NULL,xx,yy,BordGP,33);
				UINF.AddSD(SD,2);
				...
				for(j=0;j<8;j++)if(UINF.SHLMask&(1<<j)){
					...
					UINF.AddSD(SD,2);
					SD=UNITINF.addGPPicture(NULL,xx+1,yy,4,WeaponIcn[j]+7);
					UINF.AddSD(SD,2);
	...
	if(Description){
		...
		TextViewer* TV=UNITINF.addTextViewer(NULL,xx-4,yy-3+16,215,InfY1-UINF.Y1Line-16-16,cc,&SmallYellowFont);
		TV->SymSize+=2;
		TV->PageSize=(TV->y1-TV->y)/TV->SymSize;
		UINF.AddSD(TV,1);
		VScrollBar* SB=UNITINF.addNewGP_VScrollBar(NULL,xx+220,yy-11,InfY1-UINF.Y1Line,1,0,3,0);
		UINF.AddSD(SB,1);
		TV->AssignScroll(SB);
...
void CreateINFORM(){
	...
	INFORM.CloseDialogs();
	INFORM.addNewGP_VScrollBar(NULL,RealLx-22,60+InfAddY,364+InfY1-InfAddY-450,1,0,3,0);
	int DD1=75;
	int D=18;
	INFORM.HintFont=&SmallBlackFont1;
	INFORM.HintX=-1;
	INFORM.HintY=350;
	INFORM.addGPPicture(NULL,RealLx-InfDX-1,InfY2,BordGP,85);
	...
	/*
	TextButton* TB=INFORM.addTextButton(NULL,RealLx-InfDX+25,InfY1-DD1,GetTextByID("NationalForces"),&WhiteFont,&YellowFont,&YellowFont,0);
	TB->OnClick=&InfClick;
	TB->UserParam=0;
	TB=INFORM.addTextButton(NULL,RealLx-InfDX+25,InfY1-DD1+D,GetTextByID("DeathList"),&WhiteFont,&YellowFont,&YellowFont,0);
	TB->OnClick=&InfClick;
	TB->UserParam=1;
	TB=INFORM.addTextButton(NULL,RealLx-InfDX+25,InfY1-DD1+D*2,GetTextByID("Economica"),&WhiteFont,&YellowFont,&YellowFont,0);
	TB->OnClick=&InfClick;
	TB->UserParam=2;
	TB=INFORM.addTextButton(NULL,RealLx-InfDX+25,InfY1-DD1+D*3,GetTextByID("Doxod"),&WhiteFont,&YellowFont,&YellowFont,0);
	TB->OnClick=&InfClick;
	TB->UserParam=3;
	*/
};
...
void CreateRESSEND(){
	...
	int FDY=40;
	RESSEND.addTextButton(NULL,X0+16,Y0+5    ,STR1,&YellowFont,&YellowFont,&YellowFont,0);
	...
	GP_TextButton* OKBTN=RESSEND.addGP_TextButton(NULL,XB+9,YB+20,STR5,CBB_GPFILE,9,&WhiteFont,&YellowFont);
	...
	RESSEND.addGPPicture(NULL,XB,YB,CBB_GPFILE,8);
	RESSEND.addGPPicture(NULL,XB,YB,CBB_GPFILE,11);
	GP_TextButton* CANCELBTN=RESSEND.addGP_TextButton(NULL,XB+9,YB+20,STR6,CBB_GPFILE,9,&WhiteFont,&YellowFont);
